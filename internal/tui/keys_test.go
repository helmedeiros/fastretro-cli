...
func TestHandleBrainstormKeys_Tab(t *testing.T) {
	...
	if model.activeCol != 1 {
		t.Errorf("expected activeCol 1, got %d", model.activeCol)
	}
}

func TestHandleBrainstormKeys_TabWraps(t *testing.T) {
	m := testBrainstormModel()
	m.activeCol = 1
	...
	if model.activeCol != 0 {
		t.Errorf("expected activeCol to wrap to 0, got %d", model.activeCol)
	}
}
...
func TestHandleBrainstormKeys_ShiftTab(t *testing.T) {
	...
	if model.activeCol != 0 {
		t.Errorf("expected activeCol 0, got %d", model.activeCol)
	}
}
...
func TestHandleBrainstormKeys_L(t *testing.T) {
	m := testBrainstormModel()
	m.activeCol = 0

	result, _ := m.handleBrainstormKeys(keyMsg("l"))
	model := result.(Model)

	if model.activeCol != 1 {
		t.Errorf("l should move right, got %d", model.activeCol)
	}
}

func TestHandleBrainstormKeys_H(t *testing.T) {
	m := testBrainstormModel()
	m.activeCol = 1

	result, _ := m.handleBrainstormKeys(keyMsg("h"))
	model := result.(Model)

	if model.activeCol != 0 {
		t.Errorf("h should move left, got %d", model.activeCol)
	}
}

func TestHandleBrainstormKeys_A(t *testing.T) {
	m := testBrainstormModel()
	...
	if !model.inputMode {
		t.Error("'a' should activate input mode")
	}
}
...
func TestHandleBrainstormInput_Enter(t *testing.T) {
	...
	lastCard := model.state.Cards[len(model.state.Cards)-1]
	if lastCard.Text != "new card" {
		t.Errorf("expected card text 'new card', got %q", lastCard.Text)
	}
}
...
func TestHandleBrainstormInput_EnterEmpty(t *testing.T) {
	...
	if len(model.state.Cards) != initialCards {
		t.Error("should not add card for empty input")
	}
}
...
func TestHandleBrainstormInput_TypeChar(t *testing.T) {
	m := testBrainstormModel()
	m.inputMode = true
	m.inputText = "ab"

	result, _ := m.handleBrainstormInput(keyMsg("c"))
	model := result.(Model)

	if model.inputText != "abc" {
		t.Errorf("expected 'abc', got %q", model.inputText)
	}
}
...
func TestHandleBrainstormInput_MaxLength(t *testing.T) {
	...
	m.inputMode = true
	// Fill to 140 chars
	m.inputText = string(make([]byte, 140))
	for i := range []byte(m.inputText) {
		_ = i
	}
	// Build exactly 140 chars
	buf := make([]byte, 140)
	for i := range buf {
		buf[i] = 'a'
	}
	...
	if len(model.inputText) != 140 {
		t.Errorf("should not exceed 140 chars, got %d", len(model.inputText))
	}
}
...
func TestHandleBrainstormKeys_DelegatesToInput(t *testing.T) {
	...
	if model.inputText != "xa" {
		t.Errorf("expected 'xa', got %q", model.inputText)
	}
}

// --- Vote key handler tests ---

func TestHandleVoteKeys_CursorUp(t *testing.T) {
	m := testVoteModel()
	m.cursor = 1
...
func TestHandleVoteKeys_CursorDown(t *testing.T) {
	m := testVoteModel()
	m.cursor = 0
...
func TestHandleVoteKeys_CursorK(t *testing.T) {
	m := testVoteModel()
	m.cursor = 1
...
func TestHandleVoteKeys_CursorJ(t *testing.T) {
	m := testVoteModel()
	m.cursor = 0
...
func TestHandleVoteKeys_VoteWithEnter(t *testing.T) {
	m := testVoteModel()
	m.cursor = 1 // ungrouped card c3
...
func TestHandleVoteKeys_VoteNoBudget(t *testing.T) {
	...
	if len(model.state.Votes) != initialVotes {
		t.Error("should not add vote when budget exhausted")
	}
}

func TestHandleVoteKeys_Unvote(t *testing.T) {
	m := testVoteModel()
	m.cursor = 0 // g1, which p1 has voted for
...
func TestHandleVoteKeys_UnvoteNoVote(t *testing.T) {
	m := testVoteModel()
	m.cursor = 1 // c3, which p1 has NOT voted for
	initialVotes := len(m.state.Votes)
	...
	if len(model.state.Votes) != initialVotes {
		t.Error("should not remove any vote when none exists")
	}
}
